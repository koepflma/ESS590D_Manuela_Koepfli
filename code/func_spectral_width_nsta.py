...
def spectral_width_nsta(jday, client, list_stations, year, preprocessing_type):
    ...
    window_duration_sec = 60 # Fourier calcualation window in seconds (depends on array opening)
    average = 60 # number of windows to estimate the sample covariance (bigger than number of stations)

    # check which files already exist---------------------------------------------------------------------------------------

    file_path = '/data/wsd03/data_manuela/MtStHelens/covariance/{}/{:03d}/'.format(year,jday)
    ...
    if all(list(map(os.path.isfile,file_list))):
        ...
        for xx in list_stations:
            ...
            try:
                ...
                if npts/86400 < 0.75: # if stream contains less than 75% of data
        ...
        if len(stream) == 0: # if no station was active or not enough data
            UTC_times = np.array([t+sec for sec in np.arange(0,3600*24+1,15*60)]) # 15 min time steps over one day
        ...
        #            norm_spectral_width = spectral_width/len(stream)
